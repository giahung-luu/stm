...
const Character = (props) => {
    ...
    const { character, color, setSelectedCharacter, setShowModal, setCharacters } = props;

    const handleRemoveCharacter = async () => {
        const data = await deleteCharacter(character.id);
        if (data.id) {
            setCharacters((characters) => characters.filter((character) => character.id !== data.id));
    ...
    return (<tr>
        ...
        <td className="tw-order-t-0 tw-px-6 tw-align-middle tw-border-l-0 tw-border-r-0 tw-text-xs tw-whitespace-nowrap tw-p-4">
            <div className="tw-flex tw-items-center" onClick={() => { setShowModal(true); setSelectedCharacter(character) }}>
                <span className="tw-mr-2">
                    <i className="tw-fas tw-fa-circle tw-text-orange-500">Edit</i>{" "}
                </span>
            </div>
            <div className="tw-flex tw-items-center" onClick={handleRemoveCharacter}>
                <span className="tw-mr-2">
